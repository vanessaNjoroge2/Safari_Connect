        ...
        <div>
          ...
          <div className="card">
            ...
            {[
              ...
              ['Departure', booking.selectedBus?.departureTime ?? '-'],
              ['Arrival', booking.selectedBus?.arrivalTime ?? '-'],
            ...
            <div style={{ background: 'var(--brand-light)', border: '1px solid var(--brand-mid)', borderRadius: 'var(--r)', padding: '12px 14px', marginBottom: 18 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
                ...
                <span><strong>Passed</strong> · Booking and payment details validated before checkout</span>
              </div>
            </div>
            ...
            <button className="btn btn-primary btn-full btn-lg" onClick={handleProceed} disabled={submitting || !confirmPersonalData}>
              {submitting ? 'Creating booking...' : `Pay KES ${totalFare.toLocaleString()} via M-Pesa ->`}
            </button>
          </div>
        </div>
        ...
        <div>
          <div className="card mb-4" style={{ background: 'var(--gray-900)', border: 'none' }}>
            <div style={{ background: 'var(--brand)', borderRadius: 'var(--r)', padding: '16px 18px', marginBottom: 18 }}>
              ...
              <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 22, fontWeight: 800, color: '#fff' }}>{routeLabel}</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,.75)', marginTop: 4 }}>
                {(booking.searchQuery?.date ?? '-')} | {(booking.selectedBus?.departureTime ?? '-')}
              </div>
            </div>
