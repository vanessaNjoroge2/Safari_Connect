...
export default function OwnerHelp() {
  ...
  const runAi = async () => {
    ...
    try {
      const response = await aiAssistApi({
        prompt: `Owner support request: ${prompt}. Context article title: ${active?.title || 'none'}. Context article body: ${active?.content || 'none'}. Return concise practical guidance for SACCO operations.`,
        language: 'en',
      });
  ...
  return (
    <DashboardLayout title="Help Center" subtitle="Operations guidance and AI assistance">
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <div className="card">
            <div className="card-title">Ask AI for operations help</div>
            <div style={{ display: 'grid', gap: 10 }}>
              <textarea
                className="input"
                rows={4}
                placeholder="Describe your issue, for example: Trip confirmed but ticket QR is missing"
                value={prompt}
